...
outputfile = "Rslt/NCE_rslt_load{}.csv".format(load)
regretfile= "Rslt/NCE_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/NCE_load{}/".format(load)
log_dir = "lightning_logs/NCE_load{}/".format(load)
...
for seed in range(10):
    ...
    model = CachingPO(param=param,net=nn.Linear(8,1),loss_fn = NCE_hatc_c,init_cache =  cache,
    ...
    param=param,net=nn.Linear(8,1),loss_fn = NCE_hatc_c,init_cache =  cache,
            growth= growth,lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='NCE_hatc_c'
df.to_csv("LearningCurve/NCE_load{}_growth{}.csv".format(load,growth))