...
outputfile = "Rslt/SPO_rslt_load{}.csv".format(load)
regretfile= "Rslt/SPO_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/SPO_load{}/".format(load)
log_dir = "lightning_logs/SPO_load{}/".format(load)
...
for seed in range(10):
    ...
    model = SPO(param=param,net=nn.Linear(8,1),lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='SPO'
df.to_csv("LearningCurve/SPO_load{}.csv".format(load))