...
outputfile = "Rslt/Pairwise_rslt_load{}.csv".format(load)
regretfile= "Rslt/Pairwise_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/Pairwise_load{}/".format(load)
log_dir = "lightning_logs/Pairwise_load{}/".format(load)
...
for seed in range(10):
    ...
    model = CachingPO(param=param,net=nn.Linear(8,1),loss_fn = pairwise_loss,init_cache =  cache,
            growth= growth,tau=margin,lr=lr, seed=seed, max_epochs= max_epochs)


    trainer.fit(model, train_dl, valid_dl)
    best_model_path = checkpoint_callback.best_model_path
    model = CachingPO.load_from_checkpoint(best_model_path,
    param=param,net=nn.Linear(8,1),loss_fn = pairwise_loss,init_cache =  cache,
            growth= growth,tau=margin,lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Pairwise'
df.to_csv("LearningCurve/Pairwise_load{}_growth{}_lr{}_margin{}.csv".format(load,growth,lr, margin))