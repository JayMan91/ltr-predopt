...
outputfile = "Rslt/Twostage_rslt_load{}.csv".format(load)
regretfile= "Rslt/Twostage_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/Twostage_load{}/".format(load)
log_dir = "lightning_logs/Twostage_load{}/".format(load)
...
for seed in range(10):
    ...
    param=param,net=nn.Linear(8,1),lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Twostage'
df.to_csv("LearningCurve/Twostage_load{}.csv".format(load))