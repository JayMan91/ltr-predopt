...
outputfile = "Rslt/MAP_rslt_load{}.csv".format(load)
regretfile= "Rslt/MAP_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/MAP_load{}/".format(load)
log_dir = "lightning_logs/MAP_load{}/".format(load)
...
for seed in range(10):
    ...
    model = CachingPO(param=param,net=nn.Linear(8,1),loss_fn = MAP_hatc_c,init_cache =  cache,
    ...
    param=param,net=nn.Linear(8,1),loss_fn = MAP_hatc_c,init_cache =  cache,
            growth= growth,lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='MAP_hatc_c'
df.to_csv("LearningCurve/MAP_load{}_growth{}.csv".format(load,growth))