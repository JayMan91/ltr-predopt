...
outputfile = "Rslt/Combined_rslt_load{}.csv".format(load)
regretfile= "Rslt/Combined_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/Combined_load{}/".format(load,alpha)
log_dir = "lightning_logs/Combined_load{}/".format(load,alpha)
...
for seed in range(10):
    ...
    model = CombinedPO(alpha=alpha, param=param,net=nn.Linear(8,1),loss_fn = Listnet_loss,init_cache =  cache,
    ...
    alpha=alpha, param=param,net=nn.Linear(8,1),loss_fn = Listnet_loss,init_cache =  cache,
            growth= growth,tau=temperature ,lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Listwise'
df.to_csv("LearningCurve/Combined_load{}_growth{}_lr{}_temp{}_alpha{}.csv".format(load,growth,lr,temperature,alpha))