...
outputfile = "Rslt/Listwise_rslt_load{}.csv".format(load)
regretfile= "Rslt/Listwise_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/Listwise_load{}/".format(load)
log_dir = "lightning_logs/Listwise_load{}/".format(load)
...
for seed in range(10):
    ...
    # model = twostage_regression(param=param,net=nn.Linear(8,1),lr=lr, seed=seed, max_epochs= max_epochs)
    model = CachingPO(param=param,net=nn.Linear(8,1),loss_fn = Listnet_loss,init_cache =  cache,
            growth= growth,tau=temperature ,lr=lr, seed=seed, max_epochs= max_epochs)


    trainer.fit(model, train_dl, valid_dl)
    best_model_path = checkpoint_callback.best_model_path
    model = CachingPO.load_from_checkpoint(best_model_path,
    param=param,net=nn.Linear(8,1),loss_fn = Listnet_loss,init_cache =  cache,
            growth= growth,tau=temperature ,lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Listwise'
df.to_csv("LearningCurve/Listwise_load{}_growth{}_lr{}_temp{}.csv".format(load,growth,lr,temperature))