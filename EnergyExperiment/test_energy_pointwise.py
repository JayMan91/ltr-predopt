...
outputfile = "Rslt/Pointwise_rslt_load{}.csv".format(load)
regretfile= "Rslt/Pointwise_Regret_load{}.csv".format(load)
ckpt_dir =  "ckpt_dir/Pointwise_load{}/".format(load)
log_dir = "lightning_logs/Pointwise_load{}/".format(load)
...
for seed in range(10):
    ...
    model = CachingPO(param=param,net=nn.Linear(8,1),loss_fn = pointwise_loss,init_cache =  cache,
    ...
    param=param,net=nn.Linear(8,1),loss_fn = pointwise_loss,init_cache =  cache,
            growth= growth,lr=lr, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Pointwise'
df.to_csv("LearningCurve/Pointwise_load{}_growth{}_lr{}.csv".format(load,growth,lr))