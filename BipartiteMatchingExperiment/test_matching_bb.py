...
regretfile= "Rslt/Blackbox_{}_Regret.csv".format(exp)
ckpt_dir =  "ckpt_dir/Blackbox_{}/".format(exp)
log_dir = "lightning_logs/Blackbox_{}/".format(exp)
...
for seed in range(10):
    ...
    model = Blackbox( param=param, mu =mu,lr=lr, seed=seed, max_epochs=max_epochs )
    trainer.fit(model, train_dl, valid_dl)
    best_model_path = checkpoint_callback.best_model_path
    model = Blackbox.load_from_checkpoint(best_model_path,
     param=param, mu =mu,lr=lr, seed=seed, max_epochs=max_epochs)
...
df['model'] ='Blackbox'
df.to_csv("LearningCurve/Blackbox_{}.csv".format(exp))