...
regretfile= "Rslt/Pointwise_{}_Regret.csv".format(exp)
ckpt_dir =  "ckpt_dir/Pointwise_{}/".format(exp)
log_dir = "lightning_logs/Pointwise_{}/".format(exp)
...
for seed in range(10):
    ...
    model = CachingPO( param=param,loss_fn= pointwise_loss,init_cache=cache, growth=growth,
     lr=lr, seed=seed, max_epochs=max_epochs )
    trainer.fit(model, train_dl, valid_dl)
    best_model_path = checkpoint_callback.best_model_path
    model = CachingPO.load_from_checkpoint(best_model_path,
    param=param,loss_fn= pointwise_loss,init_cache=cache, growth=growth,
     lr=lr, seed=seed, max_epochs=max_epochs)
...
df['model'] = 'Pointwise'
df.to_csv("LearningCurve/Pointwise_{}_growth{}_lr{}.csv".format(exp,growth,lr))