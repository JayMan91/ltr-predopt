...
regretfile= "Rslt/Twostage_{}_Regret.csv".format(exp)
ckpt_dir =  "ckpt_dir/Twostage_{}/".format(exp)
log_dir = "lightning_logs/Twostage_{}/".format(exp)
...
for seed in range(10):
    ...
    model = twostage_regression( param=param, lr=lr, seed=seed, max_epochs=max_epochs )
    trainer.fit(model, train_dl, valid_dl)
    best_model_path = checkpoint_callback.best_model_path
    model = twostage_regression.load_from_checkpoint(best_model_path,
    param=param, lr=lr, seed=seed, max_epochs=max_epochs)
...
df['model'] ='Twostage'
df.to_csv("LearningCurve/Twostage_{}.csv".format(exp))