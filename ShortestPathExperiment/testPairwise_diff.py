...
outputfile = "Rslt/Pairwisediff_rslt.csv"
regretfile= "Rslt/Pairwisediff_Regret.csv"
...
for seed in range(10):
    ...
    model = CachingPO(init_cache=init_cache, loss_fn= pairwise_diffloss, net=nn.Linear(5,40), growth= growth,
    ...
    init_cache=init_cache,loss_fn=pairwise_diffloss, net=nn.Linear(5,40), growth= growth,lr= lr,l1_weight=l1_weight, seed=seed, )
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Pairwise_diff'
df.to_csv("LearningCurve/Pairwise_diff_data_N_{}_noise_{}_deg_{}_growth{}_lr{}.csv".format(N,noise,deg,growth,lr))