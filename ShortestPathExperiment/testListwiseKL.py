...
outputfile = "Rslt/ListwiseKL_rslt.csv"
regretfile= "Rslt/ListwiseKL_Regret.csv"
ckpt_dir =  "ckpt_dir/ListwiseKL/"
log_dir = "lightning_logs/ListwiseKL/"
...
for seed in range(10):
    ...
    model = CachingPO(init_cache=init_cache,loss_fn= Listnet_KLloss, tau=temperature, net=nn.Linear(5,40), 
    ...
    init_cache=init_cache,loss_fn= Listnet_KLloss, net=nn.Linear(5,40), 
    growth= growth, lr= lr,tau=temperature,l1_weight=l1_weight, seed=seed, )
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Listwise(KL)'
df.to_csv("LearningCurve/ListwiseKL_data_N_{}_noise_{}_deg_{}_growth{}.csv".format(N,noise,deg,growth))