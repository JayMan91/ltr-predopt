...
outputfile = "Rslt/MAP_rslt.csv"
regretfile= "Rslt/MAP_Regret.csv"
ckpt_dir =  "ckpt_dir/MAP/"
log_dir = "lightning_logs/MAP/"
...
for seed in range(10):
    ...
    model = CachingPO(init_cache=init_cache,  loss_fn= MAP_hatc_c, net=nn.Linear(5,40), growth= growth,
    ...
    init_cache=init_cache, loss_fn=  MAP_hatc_c, net=nn.Linear(5,40), growth= growth,lr= lr,l1_weight=l1_weight, seed=seed, )
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='MAP_hatc_c'
df.to_csv("LearningCurve/MAP_data_N_{}_noise_{}_deg_{}_growth{}.csv".format(N,noise,deg,growth))