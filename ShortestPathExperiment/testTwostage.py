...
regretfile= "Rslt/Twostage_Regret.csv"
...
for seed in range(10):
    ...
    model = twostage_regression(net=nn.Linear(5,40), lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs)
    trainer.fit(model, train_dl, valid_dl)
    best_model_path = checkpoint_callback.best_model_path
    model = twostage_regression.load_from_checkpoint(best_model_path,
    net=nn.Linear(5,40), lr= lr, seed=seed)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Twostage'
df.to_csv("LearningCurve/Twostage_data_N_{}_noise_{}_deg_{}.csv".format(N,noise,deg))