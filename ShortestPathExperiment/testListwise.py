...
outputfile = "Rslt/Listwise_rslt.csv"
regretfile= "Rslt/Listwise_Regret.csv"
...
for seed in range(10):
    ...
    model = CachingPO(init_cache=init_cache,loss_fn=Listnet_loss, tau=temperature, net=nn.Linear(5,40), 
    ...
    init_cache=init_cache,loss_fn=Listnet_loss, tau=temperature,net=nn.Linear(5,40), 
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Listwise'
df.to_csv("LearningCurve/Listwise_data_N_{}_noise_{}_deg_{}_growth{}_lr{}_temp{}.csv".format(N,noise,deg,growth,lr, temperature))