...
outputfile = "Rslt/Blackbox_rslt.csv"
regretfile= "Rslt/Blackbox_Regret.csv"
...
for seed in range(10):
    ...
    model = Blackbox(net=nn.Linear(5,40), mu=mu, lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs)
    trainer.fit(model, train_dl, valid_dl)
    best_model_path = checkpoint_callback.best_model_path
    model = Blackbox.load_from_checkpoint(best_model_path,
    net=nn.Linear(5,40), mu=mu, lr= lr,l1_weight=l1_weight, seed=seed, max_epochs= max_epochs)
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Blackbox'
df.to_csv("LearningCurve/Blackbox_data_N_{}_noise_{}_deg_{}.csv".format(N,noise,deg))