...
outputfile = "Rslt/Pairwise_rslt.csv"
regretfile= "Rslt/Pairwise_Regret.csv"
...
for seed in range(10):
    ...
    model = CachingPO(init_cache=init_cache,loss_fn=pairwise_loss, net=nn.Linear(5,40), growth= growth,tau=margin,
    ...
    init_cache=init_cache,loss_fn=pairwise_loss, net=nn.Linear(5,40), growth= growth,tau=margin,lr= lr,l1_weight=l1_weight, seed=seed, )
...
df = pd.DataFrame({"step": steps,'wall_time':walltimes,  "val_regret": regrets,
 "val_mse": mses })
df['model'] ='Pairwise'
df.to_csv("LearningCurve/Pairwise_data_N_{}_noise_{}_deg_{}_growth{}_lr{}_margin{}.csv".format(N,noise,deg,growth,lr, margin))